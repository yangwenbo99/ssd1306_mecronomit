...
NUM_WCOL = 16
NUM_IPGE = 8
buf = bytearray(NUM_IPGE)  # we break each line to two

def set_pos(col=0, page=0):
    i2c.write(ADDR, b'\x00' + bytearray((0xb0 | page,)))
    c1, c2 = col & 0x0F, col >> 4
    i2c.write(ADDR, b'\x00' + bytearray((0x00 | c1,)))# lower start column address
    i2c.write(ADDR, b'\x00' + bytearray((0x10 | c2,)))# upper start column address
...
def add_text(x, y, text, draw=1): # Simula original input
    if draw:
        put_text(y, text, x)
